...
class ResponsableController extends AbstractController
{
    ...
    private function getCurrentParapharmacie(): Parapharmacie
    {
        ...
        $parapharmacieId = $parapharmacie->getId();
        if ($parapharmacieId === null) {
            $this->addFlash('error', 'Parapharmacie associée invalide. Veuillez configurer votre parapharmacie.');
            throw new AccessDeniedException('Parapharmacie associée invalide');
        }

        try {
            if ($this->entityManager->contains($parapharmacie)) {
                $this->entityManager->refresh($parapharmacie);
            } else {
        ...
        if (!$parapharmacie) {
            $user->setParapharmacie(null);
            $user->setPremiereConnexion(true);
            $this->entityManager->flush();

            $this->addFlash('error', 'Parapharmacie associée introuvable. Veuillez la configurer ou en rejoindre une.');
            throw new AccessDeniedException('Parapharmacie associée introuvable');
        }

        return $parapharmacie;
    }

    private function redirectAfterParapharmacieAccessDenied(): Response
    {
        if ($this->getUser() instanceof ResponsableParapharmacie) {
    ...
    public function dashboard(
        ...
        try {
            ...
            return $this->redirectAfterParapharmacieAccessDenied();
        }
