...
class QuizController extends AbstractController
{
    ...
    public function adminIndex(Request $request, QuizRepository $quizRepository): Response
    {
        ...
        $qb = $quizRepository->createQueryBuilder('q')
            ->addSelect('(SELECT COUNT(q_count_admin.id) FROM App\Entity\Question q_count_admin WHERE q_count_admin.quiz = q) AS HIDDEN questions_count');
        ...
        if ($minQuestions > 0) {
            $qb->andWhere('(SELECT COUNT(q_min.id) FROM App\Entity\Question q_min WHERE q_min.quiz = q) >= :minQuestions')
    ...
    public function frontQuizList(QuizRepository $quizRepository, Request $request): Response
    {
        ...
        $qb = $quizRepository->createQueryBuilder('q')
            ->addSelect('(SELECT COUNT(q_count_front.id) FROM App\Entity\Question q_count_front WHERE q_count_front.quiz = q) AS HIDDEN questions_count');
        ...
        if ($category !== '') {
            $qb->andWhere('EXISTS (SELECT 1 FROM App\Entity\Question q_cat WHERE q_cat.quiz = q AND LOWER(q_cat.category) = :category)')
                ->setParameter('category', $category);
        }

        if ($duration === 'quick') {
            $qb->andWhere('(SELECT COUNT(q_dur_quick.id) FROM App\Entity\Question q_dur_quick WHERE q_dur_quick.quiz = q) < 6');
        } elseif ($duration === 'medium') {
            $qb->andWhere('(SELECT COUNT(q_dur_medium.id) FROM App\Entity\Question q_dur_medium WHERE q_dur_medium.quiz = q) BETWEEN 6 AND 10');
        } elseif ($duration === 'long') {
            $qb->andWhere('(SELECT COUNT(q_dur_long.id) FROM App\Entity\Question q_dur_long WHERE q_dur_long.quiz = q) > 10');
        }
