...
class QuizController extends AbstractController
{
    ...
    public function generateAiQuiz(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $quiz = new Quiz();
            $quiz->setName((string) $generatedQuiz['name']);
            $quiz->setDescription((string) $generatedQuiz['description']);
            ...
            foreach (array_values($questions) as $index => $questionData) {
                ...
                $question->setType((string) $questionData['type']);
                $question->setCategory((string) $questionData['category']);
                $question->setReverse((bool) $questionData['reverse']);
    ...
    {
        if ($this->isCsrfTokenValid('delete' . $quiz->getId(), (string) $request->request->get('_token'))) {
            $em->remove($quiz);
            $em->flush();
    ...
    public function saveProgress(
        ...
        $attempt->setAnswersJson(json_encode($sanitizedAnswers, JSON_UNESCAPED_UNICODE) ?: null);
    ...
    public function submit(
        ...
        if ($attempt instanceof QuizAttempt) {
            ...
            $attempt->setAnswersJson(json_encode($answers, JSON_UNESCAPED_UNICODE) ?: null);
            $attempt->setDetectedCategoriesJson(json_encode($detectedProblems, JSON_UNESCAPED_UNICODE) ?: null);
    ...
    /**
     * @param list<string> $detectedProblems
